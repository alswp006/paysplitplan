...
> **공유의 존재 신탁은 `.ai-factory/apps-in-toss-essential.txt` §6이다.** `share`의 파라미터는
> `message` 하나이고(`text`·`url`·`title` 같은 필드는 **없다**), 앱 코드는 SDK를 직접 부르지 말고
> `src/lib/share.ts`의 `shareApp({ message, path? })`를 쓴다. 아래 표의 예제 이름은 "어디를 참고할까"
> 용이지 시그니처의 근거가 아니다 — 시그니처는 신탁 파일에서 확인하라.
...
| **공유 (텍스트)** | `with-share-text` | RN/Web | `shareApp({ message })` — SDK 원형은 `share({ message })` |
| **공유 (URL/딥링크)** | `with-share-link` | RN/Web | `shareApp({ message, path })` — `getTossShareLink` + `share({ message })` |
